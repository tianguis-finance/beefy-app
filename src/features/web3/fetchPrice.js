...
const fetchTokens = async () => {
  ...
  try {
    const response = await axios.get(`https://bodega.tianguis.finance/prices?_=${cacheBuster}`);
...
const fetchLPs = async () => {
  ...
  try {
    const response = await axios.get(`https://bodega.tianguis.finance/lps?_=${cacheBuster}`);
    return response.data;
  } catch (err) {
    console.error(err);
    return {};
